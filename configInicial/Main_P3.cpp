...
int main() {
 ...
 //     
	...
	glm::mat4 projection=glm::mat4(1); // Se definen matrices
	...
	while (!glfwWindowShouldClose(window)) //Se definen las proyecciones
	{
		...
		//NOTA:(glm::vec3) que especifica las distancias a mover en cada eje: glm::vec3(tx, ty, tz), en otras palabras alcance de la vista
		
		//Código para comunicar la información al shader
		//Sintaxis
		//variable nombre = funcionParaComunicarce (deDondeProvienen "nomVariable");
		...
		model = glm::mat4(1);//Inicializa una nueva matriz de transformacion para el segundo objeto
		model = glm::translate(model, glm::vec3(6.0f, 0.0f, 0.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(0.0f, 1.0f, 1.0f));//Modificaciones
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); //Manda informacion al shader, cualquier funcion debe ir antes
		glDrawArrays(GL_TRIANGLES, 0, 36); //Se vuelve a dibujar el objeto con la nueva transformacion

		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-6.0f, -1.0f, 3.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(2.0f, 0.0f, 1.0f));//Modificaciones
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
